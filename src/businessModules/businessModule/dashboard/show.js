        ...
        <div className="showCampaignCard">
          <div>
            <div className="objective">
              <span> {campaign.campaign_objective.toLocaleUpperCase()}</span>
              <span>
                <button onClick={() => handleCampaignCardclick()}>Edit</button>
                <button
                  onClick={() => {
                    handleDelete();
                  }}
            ...
            <div className="ShowcampaignName">{campaign.name} </div>
            <br />
            <span>
              <CgCalendarDates />
              {campaign.start_date.slice(0, 10)} - <CgCalendarDates />
              {campaign.end_date.slice(0, 10)}
            </span>{" "}
            <span
              className={
                campaign.status === "pre-launched" ? "pre-launch" : "launch"
              }
            >
              {campaign.status}
            </span>
              ...
              <div>
                ...
                <span>
                  {item.influencer.instagram ? (
                    <>{item.influencer.instagram.follower_count}</>
                  ) : null}
                  {item.influencer.facebook ? (
                    <> {item.influencer.facebook.follower_count}</>
                  ) : null}
                  {item.influencer.twitter ? (
                    <> {item.influencer.twitter.follower_count}</>
                  ) : null}
                </span>
                <span>
                  {item.influencer.instagram ? (
                    <>
                      {" "}
                      <FaRupeeSign /> {item.influencer.instagram.cost}
                    </>
                  ) : null}
                  {item.influencer.facebook ? (
                    <>
                      {" "}
                      <FaRupeeSign /> {item.influencer.facebook.cost}
                    </>
                  ) : null}
                  {item.influencer.twitter ? (
                    <>
                      {" "}
                      <FaRupeeSign /> {item.influencer.twitter.cost}
                    </>
                  ) : null}
                </span>
              </div>
                    ...
                    {item.status === "accept" || item.status === "reject" ? (
                      ...
                      <div className="acceptButton">
                        <button
                          ...
                          onClick={() => {
                            handleInfluencerAccept(item.influencer._id);
                          }}
                        >
                          Accept
                        </button>
                        <button
                          className="editButton cancle"
                          onClick={() => {
                            handleInfluencerReject(item.influencer._id);
                          }}
                        >
                          Reject
                        </button>
